...
def main():
	...
	while run:
		...
		clock.tick(60)
		pygame.display.set_caption(f"Camera, fps: {clock.get_fps():.0f}")
...
def draw(draw_rect):
	for x in range(int(draw_rect.x / TILE_WIDTH), int(draw_rect.x / TILE_WIDTH) + int(draw_rect.width / TILE_WIDTH) + 1):
		for y in range(int(draw_rect.y / TILE_HEIGHT), int(draw_rect.y / TILE_HEIGHT) + int(draw_rect.height / TILE_HEIGHT) + 1):
			color = (int((x*y/(SCREEN_WIDTH*TILE_WIDTH))*255), int((x*y/(SCREEN_WIDTH*TILE_WIDTH))*255), int((x*y/(SCREEN_WIDTH*TILE_WIDTH))*255))
			pygame.draw.rect(camera, color, ((x * TILE_WIDTH, y * TILE_HEIGHT), (TILE_WIDTH, TILE_HEIGHT)))
